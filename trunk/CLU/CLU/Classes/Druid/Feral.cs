                ...
                return new PrioritySelector(
                    new Decorator(ret => Common.Units() > 2,
                                  new PrioritySelector(
                                      ...
                                      Spell.CastSpell("Thrash", ret => Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds < 3 && Unit.TimeToDeath(Me.CurrentTarget) >= 6, "Thrash"),
                                      Spell.CastSpell("Swipe", ret => Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds >= 3 || Unit.TimeToDeath(Me.CurrentTarget) < 6, "Swipe")
                                      )
                        ),
                    new Decorator(ret => Common.Units() <= 2,
                                  new PrioritySelector(
                                      Spell.CastSpell("Faerie Fire", ret => SpellManager.Spells["Faerie Fire"].CooldownTimeLeft.TotalSeconds < 1 && !StyxWoW.Me.CurrentTarget.HasAura("Weakened Armor") || (StyxWoW.Me.CurrentTarget.HasAura("Weakened Armor") && StyxWoW.Me.CurrentTarget.GetAuraByName("Weakened Armor").StackCount < 3), "Faerie Fire"),
                    ...
                    //We only use Trinkets && Hands here
                                      new Decorator(ret => Buff.PlayerHasBuff("Tiger's Fury") && Me.CurrentTarget != null && Unit.UseCooldowns(),
                                          new PrioritySelector(
                                              Item.UseTrinkets(),
                                              Item.UseEngineerGloves()
                                              )), // Thanks Kink#
                    ...
                    //Use Racials!
                                      ...
                                      Spell.CastSpell("Trash", ret => Buff.PlayerHasActiveBuff("Clearcasting") && Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds < 3 && !Buff.PlayerHasBuff("Dream of Cenarius"), "Thrash"),
                                      Spell.CastSpell("Savage Roar", ret => Buff.PlayerBuffTimeLeft("Savage Roar") <= 1 || (Buff.PlayerBuffTimeLeft("Savage Roar") <= 3 && StyxWoW.Me.ComboPoints > 0) && StyxWoW.Me.CurrentTarget.HealthPercent < 25, "Savage Roar"),

                    //Only use this shit if we have got DoC
                                      new Decorator(ret => SpellManager.HasSpell("Dream of Cenarius"),
                                      ...
                                      new Decorator(
                                          ret => (SpellManager.HasSpell("Dream of Cenarius") && StyxWoW.Me.ComboPoints >= 5 && StyxWoW.Me.CurrentTarget.HealthPercent <= 25 && Buff.PlayerHasBuff("Dream of Cenarius")) || (!SpellManager.HasSpell("Dream of Cenarius") && Buff.PlayerHasBuff("Berserk") && StyxWoW.Me.CurrentTarget.HealthPercent <= 25) || Unit.TimeToDeath(Me.CurrentTarget) <= 40,
                                          Item.UseItem(76089)
                                          ),
                    ...
                    //Only use this shit if we have got DoC
                                      ...
                                      Spell.CastSpell("Trash", ret => Buff.PlayerHasActiveBuff("Clearcasting") && Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds < 3, "Thrash"),
                                      ...
                                      new Decorator(ret => StyxWoW.Me.CurrentTarget.MeIsBehind || BossList.CanShred.Contains(Unit.CurrentTargetEntry) || (TalentManager.HasGlyph("Shred") && (StyxWoW.Me.HasAura(5217) || StyxWoW.Me.HasAura(106951))),
                                      ...
                                      new Decorator(ret => Common.CanRavage,
                                          new PrioritySelector(
                                      ...
                                      Spell.CastSpell("Trash", ret => StyxWoW.Me.ComboPoints >= 5 && Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds < 6 && (Buff.PlayerHasBuff("Tiger's Fury") || Buff.PlayerHasBuff("Berserk")), "Thrash"),
                                      Spell.CastSpell("Trash", ret => StyxWoW.Me.ComboPoints >= 5 && Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds < 6 && ((SpellManager.HasSpell("Tiger's Fury") && SpellManager.Spells["Tiger's Fury"].CooldownTimeLeft.TotalSeconds < 3.0) || !SpellManager.HasSpell("Tiger's Fury")), "Thrash"),
                                      Spell.CastSpell("Trash", ret => StyxWoW.Me.ComboPoints >= 5 && Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds < 6 && Common.TimetoEnergyCap <= 1.0, "Thrash"),
                                      new Decorator(ret => !(StyxWoW.Me.ComboPoints >= 5 && Buff.TargetDebuffTimeLeft("Thrash").TotalSeconds < 6) && Common.CanRavage,
                                          new PrioritySelector(
                    ...
                    //Rip for Dumping CB
                                      Spell.CastSpell("Rip", ret => StyxWoW.Me.ComboPoints >= 5 && (!Buff.TargetHasDebuff("Rip") || Buff.TargetDebuffTimeLeft("Rip").TotalSeconds < 2.0), "Rip")
                                      )));
            }
        }
    }
