                ...
                return new PrioritySelector(
                           ...
                           new Decorator(ret => Unit.IsTargetWorthy(Me.CurrentTarget) && Me.IsWithinMeleeRange,//Check for the damn range, we don't want to pop anything when the destination is shit away
                                         new PrioritySelector(
                                             ...
                                             Spell.CastSelfSpell("Empower Rune Weapon", ret => Me.CurrentTarget != null && CLUSettings.Instance.DeathKnight.UseEmpowerRuneWeapon && _runeCalculus > 8 && !Buff.UnitHasHasteBuff(Me), "Empower Rune Weapon")
                                         )
                                        ),
                           //Aoe
                           ...
                           Spell.CastAreaSpell("Unholy Blight", 10, false, 3, 0.0, 0.0, 	ret => Me.CurrentTarget != null && (!BossList.IgnoreAoE.Contains(Unit.CurrentTargetEntry) && Unit.EnemyUnits.Count() >= 3), "Unholy Blight (Spreading dem dezeazez"),
                           ...
                           Spell.CastSpell("Frost Strike", ret => Me.CurrentTarget, 	ret => !IsWieldingBigWeapon() && Buff.PlayerHasBuff("Killing Machine"), "Frost Strike (Dual Wield Killing Machine)"),
                           Spell.CastSpell("Obliterate", ret => Me.CurrentTarget, 		ret => IsWieldingBigWeapon() && Buff.PlayerHasBuff("Killing Machine"), "Frost Strike (2 Hand Killing Machine)"),
                           ...
                           Spell.CastSpell("Plague Leech", ret => Me.CurrentTarget, 	ret => (Buff.TargetDebuffTimeLeft("Blood Plague").TotalSeconds > 0.5 || Buff.TargetDebuffTimeLeft("Frost Fever").TotalSeconds > 0.5) && (Spell.RuneCooldown(1) > 1 && Spell.RuneCooldown(2) > 1 && Spell.RuneCooldown(5) > 1 && Spell.RuneCooldown(6) > 1 && (Spell.RuneCooldown(3) > 1 && Spell.RuneCooldown(4) == 0 || Spell.RuneCooldown(3) == 0 && Spell.RuneCooldown(4) > 1)), "Plague Leech (Refreshed a depleted Rune)"), //Don't waste it on Unholy Runes
                    ...
                    new PrioritySelector(
                        ...
                        new Decorator(
                            ...
                            new PrioritySelector(
                                ...
                                Spell.CastSpell("Death Siphon", 			ret => Me.HealthPercent < CLUSettings.Instance.DeathKnight.DeathStrikeEmergencyPercent, "Death Siphon"),//Replaces Death Strike; Heals for 100% damage dealt
